...
# 메인 생성 함수
# ══════════════════════════════════════════════════════════
def generate_quote(quote_type: str, info: dict) -> Path:
    """
    견적서 생성 → 파일 경로 반환
    quote_type: 'awning' | 'glamping' | 'both'
    ※ 기본값(both): v6처럼 천막·어닝 + 글램핑 두 시트 모두 포함
    """
    wb = Workbook()

    # ★ 자동 계산 강제 ON (수량/단가 입력 즉시 반영)
    wb.calculation.calcMode = 'auto'
    wb.calculation.fullCalcOnLoad = True
    wb.calculation.forceFullCalc = True

    if quote_type == 'awning':
        # 천막·어닝 시트만
        ws = wb.active
        ...
        # ★ both (기본) — v6와 동일: 두 시트 모두 한 파일에
        # 천막·어닝 시트 (먼저)
        ws1 = wb.active
        ws1.title = '천막·어닝 구조물 견적서'
        info1 = dict(info)
        info1['company']   = '우성어닝천막공사캠프시스템'
        info1['biz_type']  = '제조업, 건설업'
        info1['item_desc'] = '천막·어닝·방수포·구조물'
        # 글램핑용 별도 견적번호 (A→G 접두어 교체)
        glamping_no = info['quote_no'].replace('-A', '-G')
        build_sheet(ws1, info1, AWNING_SECTIONS, AWNING_NOTES)

        # 글램핑 시트 (두번째)
        ws2 = wb.create_sheet('글램핑 텐트 견적서')
        info2 = dict(info)
        info2['quote_no']  = glamping_no
        info2['company']   = '우성글램핑N텐트'
        info2['biz_type']  = '제조업, 도소매'
        info2['item_desc'] = '텐트·글램핑·캠핑·금속·섬유'
        build_sheet(ws2, info2, GLAMPING_SECTIONS, GLAMPING_NOTES)

        # 기본 활성 시트 = 천막·어닝
        wb.active = ws1

    # 파일명: {날짜}_{견적번호}_{고객명}.xlsx
...
def interactive_input() -> tuple:
    ...
    qtype_map = {'1':'awning','2':'glamping','3':'both'}
    qtype = qtype_map[t]
    prefix = 'A' if qtype == 'awning' else ('G' if qtype == 'glamping' else 'A')
    quote_no = get_next_quote_number(qtype)

    print(f"\n견적번호: {quote_no}")
    customer  = input("고객명 (업체명): ").strip()
    address   = input("현장 주소:       ").strip()
    phone     = input("연락처:          ").strip()
    work_date = input("시공 희망일 (예: 2026-05-01, Enter=미정): ").strip() or '미정'
...
def main():
    ...
    parser.add_argument('--type', choices=['awning','glamping','both'],
                        default='both',
                        help='견적 종류 (awning/glamping/both, 기본값: both)')
    parser.add_argument('--customer', default='', help='고객명')
    parser.add_argument('--address',  default='', help='현장 주소')
    parser.add_argument('--phone',    default='', help='연락처')
    parser.add_argument('--date',     default='미정', help='시공 희망일')
    parser.add_argument('--list',     action='store_true', help='저장된 견적 목록')
    args = parser.parse_args()

    if args.list:
        list_quotes(); return

    # --customer 없이 --type만 지정한 경우 대화형으로 전환
    if args.customer == '' and not any([args.address, args.phone, args.date != '미정']):
        qtype, info = interactive_input()
    else:
        qtype = args.type
        # 'both' 타입은 A 번호로 채번 (글램핑은 A→G 자동 교체)
        quote_no = get_next_quote_number('awning' if qtype == 'both' else qtype)
        info = {
            'quote_no' : quote_no,
    ...
    print("\n생성 중...")
    out_path = generate_quote(qtype, info)

    print(f"\n✅ 완료!")
    print(f"   파일: {out_path}")
    print(f"   견적번호: {info['quote_no']}")
    if info.get('customer'):
        print(f"   고객명: {info['customer']}")

    # LibreOffice PDF 변환 (설치된 경우)
    if HAS_LIBREOFFICE:
        ans = input("\nPDF로도 저장할까요? (y/N): ").strip().lower()
        if ans == 'y':
            pdf_dir = out_path.parent
            subprocess.run([
                'libreoffice', '--headless', '--convert-to', 'pdf',
                '--outdir', str(pdf_dir), str(out_path)
            ], capture_output=True)
            pdf_path = pdf_dir / (out_path.stem + '.pdf')
            if pdf_path.exists():
                print(f"   PDF: {pdf_path}")
            else:
                print("   PDF 변환 실패 (LibreOffice 오류)")
